...
class Carousel extends React.Component {
  ...
  handleMouseMove = e => {
    ...
    carousel.firstChild.style.transform = `translateX(${transLeftOffset +
      walk}px)`;
  };
  ...
  handleSnap = () => {
    ...
    const end =
      _data.length * (itemWidth + 2 * itemSideOffsets) - carousel.offsetWidth;

    // (3) check if we passing from threshold ( handeling Snap To Sides )
    if (tempThresholdOffset > 0 || Math.abs(tempThresholdOffset) >= end + carousel.offsetWidth * 0.75) {
      this.setState({ isDown: false });
      carousel.firstChild.style.cssText = `
